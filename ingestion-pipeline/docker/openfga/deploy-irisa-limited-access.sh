...
# Deploy OpenFGA authorization model for Irisa limited access
# This script should be run after the OpenFGA service is up and running
...
OPENFGA_TOKEN=$(curl -s -X POST http://keycloak:8080/realms/iceberg/protocol/openid-connect/token \
...
MODEL_RESPONSE=$(curl -s -X POST http://openfga:8080/stores/$STORE_ID/models \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $OPENFGA_TOKEN" \
...
TUPLES_RESPONSE=$(curl -s -X POST http://openfga:8080/stores/$STORE_ID/authorization-models/$MODEL_ID/write \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $OPENFGA_TOKEN" \
